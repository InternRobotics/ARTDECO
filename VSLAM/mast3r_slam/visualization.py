...
from VSLAM.mast3r_slam.evaluate import as_SE3
from VSLAM.thirdparty.in3d.in3d.camera import Camera, ProjectionMatrix, lookat
from VSLAM.thirdparty.in3d.in3d.pose_utils import translation_matrix
from VSLAM.thirdparty.in3d.in3d.color import hex2rgba
from VSLAM.thirdparty.in3d.in3d.geometry import Axis
from VSLAM.thirdparty.in3d.in3d.viewport_window import ViewportWindow
from VSLAM.thirdparty.in3d.in3d.window import WindowEvents

from moderngl_window import resources
from moderngl_window.timers.clock import Timer

from VSLAM.ImageFrame import Mode
from VSLAM.mast3r_slam.geometry import get_pixel_coords
from VSLAM.mast3r_slam.visualization_utils import (
    Frustums,
    Lines,
    depth2rgb,
    image_with_text,
)
from VSLAM.thirdparty.in3d.in3d.image import Image
...
class Window(WindowEvents):
    ...
    def render(self, t: float, frametime: float):
        ...
        h, w  = curr_frame.img.shape[1:]

        self.frustums.make_frustum(h, w)

        color_img = inverse_normalize(curr_frame.img.cpu())
        self.curr_img_np = color_img.numpy()
        ...
        for kf_idx in dirty_idx:
            ...
            h, w = keyframe.img.shape[1:]
            X = self.frame_X(keyframe)
            C = keyframe.get_average_conf().cpu().numpy().astype(np.float32)

            if "{}_{}".format(keyframe.frame_id, keyframe.cam_id) not in self.textures:
                ...
                self.textures["{}_{}".format(keyframe.frame_id, keyframe.cam_id) ] = ptex, ctex, itex
                ptex, ctex, itex = self.textures["{}_{}".format(keyframe.frame_id, keyframe.cam_id) ]

                color_img = inverse_normalize(keyframe.img.cpu())
                itex.write(color_img.numpy().astype(np.float32).tobytes())
        ...
        for kf_idx in range(N_keyframes):
            ...
            if kf_idx == N_keyframes - 1:
                color_img = inverse_normalize(keyframe.img.cpu())
                self.kf_img_np = color_img.numpy()
        ...
        if self.show_keyframe_edges:
            with self.states.lock:
                ...
                if ii.numel() > 0 and jj.numel() > 0:
                    T_WCi = pp.Sim3(self.keyframes.T_WC[ii, 0])
                    T_WCj = pp.Sim3(self.keyframes.T_WC[jj, 0])
            if ii.numel() > 0 and jj.numel() > 0:
        ...
        if self.show_curr_pointmap and len(self.keyframes) != 0:
            if config["use_calib"]:
