...
# 根据attn的分数从feat中选出nfeat个分数最高的视觉特征，座位代表性特征
def how_select_local(feat, attn, nfeat):
    ...
    # get nfeat
    if nfeat < 0:
        # 负数表示“比例保留”，比如 nfeat = -0.3 时保留 30% 的特征
        assert nfeat >= -1.0
...
# 推理视觉特征的模型，由mast3r的backbone和后续的head以及一些白化、求模操作构成
# freeze_backbone mast3r的backbone不需要梯度
# residual 在全连接层添加残差
# nfeat 从一所有视觉特征中选多少个特征作为代表特征
# featweights 选代表特征的分数计算方式【求模】
class RetrievalModel(nn.Module):
    def __init__(self, backbone, freeze_backbone=1, prewhiten=None, hdims=[1024], residual=False, postwhiten=None,
        ...
        # 构造一个backbone后的全连接head
        self.residual = residual
        self.projector = self.build_projector(hdims, residual)

        # 后白化
        self.dim = hdims[-1] if len(hdims) > 0 else self.backbone_dim  # 当前类输出的维度
    ...
    # 根据输入的hdim的长度和size线性映射层  enc_embed_dim->hdims[-1]
